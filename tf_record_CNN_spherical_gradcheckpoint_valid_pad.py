...
def tf_record_CNN_spherical(tone_version, itd_tones, ild_tones, manually_added, freq_label, sam_tones, transposed_tones,
                            precedence_effect, narrowband_noise, all_positions_bkgd, background_textures, testing,
                            branched, zero_padded, stacked_channel, model_version, num_epochs, train_path_pattern,
                            bkgd_train_path_pattern, arch_ID, config_array, files, num_files, newpath, regularizer,
                            SNR_max=40, SNR_min=5):
    ...
    with tf.device("/cpu:0"):
        ...
        feature = parse_nested_dictionary(jsdict, is_bkgd=False)

        dataset = build_tfrecords_iterator(num_epochs, train_path_pattern, False, feature, narrowband_noise,
        ...
        # Get one background example to extract its fields
        ...
        bkgd_feature = parse_nested_dictionary(bkgd_jsdict, is_bkgd=True)

        dataset_bkgd = build_tfrecords_iterator(num_epochs, bkgd_train_path_pattern, True, bkgd_feature,
                                                narrowband_noise, manually_added, BKGD_SIZE,
                                                localization_bin_resolution, stacked_channel)


        ### Combine Signals and Backgrounds
    ...
    # Make a dict containing elev and azimuth labels (potentially more) for foreground and background for each example
    # Not sure why... It's just copying data we have already, no?
    label_dict = collections.OrderedDict()
    combined_dict_fg = collections.OrderedDict()
    combined_dict_bkgd = collections.OrderedDict()
    for k in combined_iter_dict[0]:
        if k != 'train/image' and k != 'train/image_height' and k != 'train/image_width':
            combined_dict_fg[k] = combined_iter_dict[0][k]
    for k in combined_iter_dict[1]:
        if k != 'train/image' and k != 'train/image_height' and k != 'train/image_width':
            combined_dict_bkgd[k] = combined_iter_dict[1][k]
    label_dict[0] = combined_dict_fg
    label_dict[1] = combined_dict_bkgd
    # -> label_dict[0] is the foreground, label_dict[1] is the background

    if sam_tones or transposed_tones:
        labels_batch_cost_sphere = tf.squeeze(tf.zeros_like(label_dict[0]['train/carrier_freq']))
    elif precedence_effect:
        labels_batch_cost_sphere = tf.squeeze(tf.zeros_like(label_dict[0]['train/start_sample']))
    else:
        if not tone_version:
            labels_batch_sphere = tf.add(tf.scalar_mul(tf.constant(36, dtype=tf.int32), label_dict[0]['train/elev']),
                                         label_dict[0]['train/azim'])
            # -> Tensor with the NN class number of the foreground sound for each example
        else:
            labels_batch_sphere = label_dict[0]['train/azim']
            # -> Tensor with the azimuth for each example
        labels_batch_cost_sphere = tf.squeeze(labels_batch_sphere)
        # -> apparently there's a dimension or so left over that we can remove
    ...
    if branched:
        ...
        cost2 = tf.reduce_mean(
            tf.nn.sparse_softmax_cross_entropy_with_logits(logits=out2, labels=label_dict[0]['train/class_num']))
        cost = cost1 + cost2
    else:
        cost = tf.reduce_mean(
            tf.nn.sparse_softmax_cross_entropy_with_logits(logits=out, labels=labels_batch_cost_sphere))
        ...
        # cost = mean(cross_entropy(output[batch_size,num_classes],truth[batch_size])[batch_size])[1]
        # cost = mean(cross_entropy(output[16,504],truth[16])[16])[1]
        # Take mean of every example's loss (i.e. its cross entropy) to get the batch's loss
    ...
    if branched:
        correct_pred2 = tf.equal(tf.argmax(out2, 1), tf.cast(label_dict[0]['train/class_num'], tf.int64))
    ...
    sess.run(init_op)
    if branched:
        print("Class Labels:" + str(sess.run(label_dict[0]['train/class_num'])))
        ...
        try:
            ...
            while True:
                ...
                if step % display_step == 0:
                    ...
                    loss, acc, az = sess.run([cost, accuracy, label_dict[0]['train/azim']])
    ...
    if testing:
        ...
        for stim in model_version:  # Why called stim? It's a list of model versions...
            ...
            step = 0
            try:
                eval_vars = list(label_dict[0].values())
                eval_keys = list(label_dict[0].keys())
                while True:
...
build_tfrecords_iterator():
list_of_filenames
.map(filename -> TFRecordDataset(filename))
.map(single_record -> parse_function(single_record))
.shuffle(200)
.repeat(num_epochs)
.prefetch(100)

parse_function(record):
- decode image, cast it to float32, and reshape it to original shape
- Cast all int64 dtypes to int32
- Divide the azim and elev labels by 5 (-> Why divide elevation by 5?)
- I think: replace dtype strings with tf.dtype objects...?

dataset = build_tfrecords_iterator(signal_paths)
dataset_bkgd = build_tfrecords_iterator(background_paths)

# Combine signals and backgrounds:
zip(dataset, dataset_bkgd)
.map(combine_signal_and_noise(signal_example, bkgd_example))
.shuffle(200)
.batch(16, drop_remainder=True)
.make_initializable_iterator() -> combined_iter
.get_next() -> combined_iter_dict  # I think this is so we can continue to work on one batch
# Downsample:
.unstack()
.concat()
.expand_dims() -> left, right
downsample(left, right)
.squeeze()
.split() -> left, right
tf.stack(left, right) -> downsampled_example_batch

# Build the network:
net.build(downsampled_example_batch)  # not a method in the code, but here for simplicity's sake

downsampled_example_batch
.copy_and_remove_image() -> label_dict  # not a function # contains foreground and background labels

(label_dict[fg]['elev'] * 36) + label_dict[fg]['azim']
.squeeze() -> labels_batch_cost_sphere

cross_entropy(out, labels_batch_cost_sphere)
.add_regularization(cost, regularizer) -> cost
# Evaluation:
.softmax() -> cond_dist
.argmax() -> net_pred
.equalTo(labels_batch_cost_sphere) -> correct_pred
.reduce_mean() -> accuracy
...
# Training:
sess.run(combined_iter.initializer)
sess.run(AdamOptimizer(learning_rate=1e-3)
.minimize(cost))
...
# Testing:
sess.run(combined_iter.initializer)
sess.run([net_pred, correct_pred, cond_dist, eval_vars])  (not sure what eval_vars is)
